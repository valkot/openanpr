...
#define CHECK_GREATER_THAN(expected,actual)\
{\
    try { \
        int _expected = (expected);\
        int _actual = (actual);\
        if (_expected >= _actual) {\
            char message [80];\
            sprintf (message, "expected %d was less than or equal to: %d", _expected, _actual);\
            result_.addFailure (Failure (message, name, __FILE__, __LINE__));\
        }\
    } catch(...) { \
    result_.addFailure (Failure ("Unhandled exception", name, __FILE__, __LINE__)); \
...
#define CHECK_LESS_THAN(expected,actual)\
{\
    try { \
        int _expected = (expected);\
        int _actual = (actual);\
        if (_expected <= _actual) {\
            char message [80];\
            sprintf (message, "expected %d was greater than or equal to: %d", _expected, _actual);\
            result_.addFailure (Failure (message, name, __FILE__, __LINE__));\
        }\
    } catch(...) { \
    result_.addFailure (Failure ("Unhandled exception", name, __FILE__, __LINE__)); \
...
#define CHECK_FLOATS_EQUAL(expected,actual)\
{\
    try { \
        ...
        if (fabs ((_expected)-(_actual)) > 0.001) {\
            char message [80];\
            sprintf (message, "expected %f but was: %f", (_expected), (_actual));\
            result_.addFailure (Failure (message, name, __FILE__, __LINE__));\
        }\
    } catch(...) { \
    result_.addFailure (Failure ("Unhandled exception", name, __FILE__, __LINE__)); \
